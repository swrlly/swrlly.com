/* Original code adapted from http://jsfiddle.net/ditman/8Ffrw/ */
 ...
 */
 var Star = function(x, y, maxSpeed) {
    ...
    this.theta = Math.atan(this.slope) * 180 / Math.PI;
    this.opacity = 0;
    this.speed = Math.max(Math.random() * maxSpeed, 0.1);
};
...
StarField.prototype._updateStarField = function() {
    ...
    for (i = 0; i < this.numStars; i++) {
        ...
        increment = star.speed;
        star.r += increment * 20;
        star.theta += increment / 20;
        star.x = star.r * Math.cos(star.theta);
        star.y = star.r * Math.sin(star.theta);
        
        star.opacity += increment / this.opacityIncrement;
...
StarField.prototype._renderStarField = function() {
    ...
    for (i = 0; i < this.numStars; i++) {
        star = this.starField[i];
        this.canvas.fillStyle = "rgba(179, 154, 240, " + star.opacity + ")";
 ...
 */
StarField.prototype.render = function(numStars, maxStarSpeed, opacityIncrement) {
